    ...
    while True:
        ...
        for conn in list(players.keys()):
            if conn in eliminated:
                ...
                continue
            try:
                packet = '|'.join([f"{p['id']},{p['x']},{p['y']}, {p['r']}"for c, p in players.items() if c != conn and c not in eliminated]) + '|'
...
while True:
    try:
        ...
        id_counter += 1
        players[conn] = {'id':id_counter, "x":id_counter, "x":0, 'y':0, 'r':20}
    except:
        pass